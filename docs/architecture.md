...
```
scenes/     main.tscn wires everything; car, track, camera, UI as sub-scenes
...
`scenes/track/track_02.tscn`, `scenes/ui/hud.tscn` and `scenes/main.tscn` are
produced by scripts in `tools/`. The generated `.tscn` files are committed, so
the game never depends on the tools at runtime — but the circuit is *described*
by a layout spec rather than hand-placed, and rebuilding is one command.

`main.tscn` in particular must be regenerated rather than hand-edited: instance
...
The builder prints a **closure gap**, which must be `(0, 0)` with net ±4 turns.
That is how the loop is made to actually join up — adjust straight counts until
it closes.
